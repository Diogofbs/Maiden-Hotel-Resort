...
export class ClientsComponent implements OnInit, OnDestroy {
  ...
  constructor(private app: AppComponent, private datepipe: DatePipe,
              private clientsService: ClientsService, private guestsService: GuestsService) {
  ...
  onCreateClient(){
    ...
    // send http request
    this.subscription = this.clientsService.createAndStoreClient(
  ...
  onDeleteClient(){
    ...
    // send http request
    this.subscription = this.clientsService.deleteClient(this.clients[index].id).subscribe(responseData => {
      // console.log(responseData);
  ...
  private fetchClients(){
    ...
    this.subscription = this.clientsService.fetchClients().subscribe(clients => {
  ...
  private fetchGuests() {
    this.subscription = this.guestsService.fetchGuests().subscribe(guests => {
      ...
      for (var i = 0, len = guests.length; i < len; i++) {
        this.guests.push(new Guest(guests[i].id, guests[i].firstName, guests[i].lastName, guests[i].dateOfBirth,
            guests[i].gender, guests[i].idNumber, guests[i].address, guests[i].contactNumber, guests[i].email, guests[i].status));
